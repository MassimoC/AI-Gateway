...
// MARK: built-in logging: additions BEGIN
...
// MARK: built-in logging: additions END

// MARK: content filtering: additions BEGIN
...
@description('The name of the APIM API for Content Safety')
param contentSafetyAPIName string = 'contentsafety'

@description('The relative path of the APIM API for Content Safety')
param contentSafetyAPIPath string = 'contentsafety'

@description('The display name of the APIM API for Content Safety')
param contentSafetyAPIDisplayName string = 'ContentSafety'

@description('The description of the APIM API for Content Safety')
param contentSafetyAPIDescription string = 'Content Safety API'
...
resource contentSafetyAPI 'Microsoft.ApiManagement/service/apis@2023-05-01-preview' = {
  name: contentSafetyAPIName
  parent: apimService
  properties: {
    apiType: 'http'
    description: contentSafetyAPIDescription
    displayName: contentSafetyAPIDisplayName
    format: 'openapi-link'
    path: contentSafetyAPIPath
    serviceUrl: '${contentSafetyResource.properties.endpoint}/contentsafety'
    protocols: [
      'https'
    ]
    subscriptionRequired: true
    type: 'http'
    value: contentSafetyAPISpecURL
  }
}

resource contentSafetyAPIPolicy 'Microsoft.ApiManagement/service/apis/policies@2021-12-01-preview' = {
  name: 'policy'
  parent: contentSafetyAPI
  properties: {
    format: 'rawxml'
    value: loadTextContent('contentsafety-policy.xml')
  }
}
